  ...
  const getAllParameters = async () => {
    try {
      const response = await axios.get(
        "http://localhost:5000/parameter/fuelType",
        {
          headers: {
      ...
      const res = await axios.get(
        "http://localhost:5000/parameter/transmissionType",
        {
          headers: {
            "Cache-Control": "no-cache, no-store, must-revalidate",
            Pragma: "no-cache",
  ...
  const handleAddVehicle = async () => {
    ...
    try {
      const res = await axios.post(
        "http://localhost:5000/owner/vehicle",
        formData,
        {
          headers: {
      ...
      handleClose();
      setAlertMessage(`Successfully Added ${vehicleD.number_plate} !`);
      setAlertType("success");
      setOpenAlert(true);
    } catch (error) {
      console.error("Error adding vehicle:", error);
      // Handle error (e.g., display error message)
      setAlertMessage(`Error Adding ${vehicleD.number_plate} !`);
      setAlertType("error");
      setOpenAlert(true);
    }
  };
  ...
  return (
    <div>
      ...
      <Dialog fullWidth maxWidth="md" open={open} onClose={handleClose}>
        <DialogTitle>
          {isUpdate
            ? `EDIT VEHICLE: ${vehicleD.number_plate}`
            : `ADD NEW VEHICLE`}
                  ...
                  {field.type === "text" ? (
                    <>
                      ...
                      <TextField
                        ...
                        onChange={(event) =>
                          handleChange(event.target.value, field.id)
                        }
                      />
                    </>
                  ) : field.type === "number" ? (
